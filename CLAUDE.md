...
Palette **crème / ambre** — thème actif depuis mai 2026.

| Token | Valeur | Rôle |
|-------|--------|------|
| `--primary` | `#A8810A` | Or ambré foncé — actions primaires |
| `--primary-hover` | `#8A6A08` | Hover boutons primaires |
| `--primary-soft` | `rgba(168,129,10,.10)` | Fond léger éléments actifs |
| `--primary-mid` | `rgba(168,129,10,.25)` | Bordure éléments actifs |
| `--primary-glow` | `rgba(168,129,10,.20)` | Halo hover chips/boutons |
| `--bg-main` | `#F5F2EA` | Fond global crème |
| `--bg-card` | `#FFFFFF` | Cartes, modals |
| `--bg-soft` | `#EEE9DC` | Inputs, zones secondaires |
| `--text-main` | `#1C1A10` | Texte principal |
| `--text-muted` | `#7A6A2A` | Texte secondaire |
| `--text-light` | `#B8A060` | Labels, placeholders |
| `--border` | `#DDD8CC` | Bordures légères |
| `--border-strong` | `#C8C0A8` | Bordures marquées |

**Règles critiques :**
- `btn-primary` → `color: #fff` (blanc sur fond ambre) — NE JAMAIS mettre `color: #000`
- Ne jamais hardcoder de couleurs hex — toujours utiliser les tokens `var(--...)`
- Référence visuelle complète : `today-preview.html` à la racine du projet

**Fichiers CSS :**
| Fichier | Rôle |
|---------|------|
| `src/assets/css/base.css` | Tokens CSS — SEUL fichier à modifier pour changer l'apparence globale |
| `src/assets/css/modal.css` | Système modal + patterns partagés |
| `src/assets/css/buttons.css` | Tous les variants `.btn-*` |
| `src/assets/css/components.css` | Badges, inputs, cards, tables |
| `src/assets/css/layout.css` | Structure page, sidebar, topbar |

**Système modal — règles d'uniformité :**
- Toujours `<BaseModal title="...">` — jamais de `<header class="modal-title">` manuel
- La prop `title` rend automatiquement : point ambre (`.modal-header-dot`) + titre + bouton fermer
- Sections dans `modal-body` : sections plates + `<div class="form-divider">` entre elles — **pas de `form-box`**
- Labels de section : classe `.form-label` globale — pas de `.section-label` scoped
- La prop `dark` est supprimée — ne pas l'utiliser
