...
export async function fetchAllClients() {
  ...
  const [{ data, error }, { data: badAppts }] = await Promise.all([
    supabase.from('clients').select('*').eq('organization_id', orgId).order('name', { ascending: true }),
    supabase.from('appointments').select('client_id, status').eq('organization_id', orgId).in('status', ['cancelled', 'noshow'])
  ])
  if (error) throw error

  const counts = { cancelled: {}, noshow: {} }
  for (const a of (badAppts || [])) {
    if (!a.client_id) continue
    if (a.status === 'cancelled') counts.cancelled[a.client_id] = (counts.cancelled[a.client_id] || 0) + 1
    if (a.status === 'noshow')    counts.noshow[a.client_id]    = (counts.noshow[a.client_id]    || 0) + 1
  }

  return data.map(c => {
    const noShowCount    = counts.noshow[c.id]    || 0
